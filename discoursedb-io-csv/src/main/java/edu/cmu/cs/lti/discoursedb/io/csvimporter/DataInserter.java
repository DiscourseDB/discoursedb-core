...
class DataInserter {
    ...
    public void insertData(Map<String, String> row, String csvFileName) {
        ...
        if (!row.containsKey("when")) {
            ...
            this.csvImportApplication.when
                .setTime(javax.xml.bind.DatatypeConverter.parseDateTime(row.get("when")).getTime());
        }
        ...
        DiscoursePart dp = this.csvImportApplication
            .createOrGetDiscoursePartLeaf(row.get("forum"), curDiscourse, row.get("forum_types"),
                new DataSourceInstance(row.get("forum"),
                    row.get("dataset_file") + "#" + row.get("dataset_forum_col") + "#path",
        ...
        User u = this.csvImportApplication.userService
            .createOrGetUser(curDiscourse, row.get("username"));
        ...
        if (u.getDataSourceAggregate() == null) {
            this.csvImportApplication.dataSourceService.addSource(u, new DataSourceInstance(
                row.get("id"),
                row.get("dataset_file") + "#" + row.get("dataset_id_col") + "#username",
                DataSourceTypes.OTHER, row.get("dataset_name")));
        ...
        CsvImportApplication.log
            .info(row.get("id") + " -- " + row.get("username") + " ---> " + row.get("post"));
        ...
        Contribution curContribution = this.csvImportApplication.contributionService
            .createTypedContribution(ContributionTypes.valueOf(row.get("contribution_type")));
        curContribution.setCurrentRevision(curContent);
        curContribution.setFirstRevision(curContent);
        curContribution.setStartTime(this.csvImportApplication.when.getTime());
        this.csvImportApplication.dataSourceService
            .addSource(curContribution, new DataSourceInstance(
                row.get("id"),
                row.get("dataset_file") + "#" + row.get("dataset_id_col") + "#contribution",
                DataSourceTypes.OTHER, row.get("dataset_name")));
        this.csvImportApplication.discoursepartService
            .addContributionToDiscoursePart(curContribution, dp);
        ...
        if (this.csvImportApplication.annos.size() > 0) {
            for (String anno : this.csvImportApplication.annos) {
                if (row.get("annotation_" + anno) != null && !row.get("annotation_" + anno)
                    .equals("")) {
                    ...
                    if (row.containsKey("annotationOwnerEmail")) {
                        ...
                        newAnno = this.csvImportApplication.annoService
                            .createUnownedTypedAnnotation(anno);
                    }
                    ...
                    if (row.get("annotation_" + anno).startsWith("[")) {
                        ...
                        for (JsonNode part : parts) {
                            Feature f = this.csvImportApplication.annoService
                                .createFeature(part.asText());
                            this.csvImportApplication.annoService.saveFeature(f);
                            this.csvImportApplication.annoService.addFeature(newAnno, f);
                        }
                    } else {
                        Feature f = this.csvImportApplication.annoService
                            .createFeature(row.get("annotation_" + anno));
                        this.csvImportApplication.annoService.saveFeature(f);
                        this.csvImportApplication.annoService.addFeature(newAnno, f);
                    }
                }
        ...
        if (row.containsKey("jsonAnnotations")) {
            try {
            	this.csvImportApplication.annoService.addAnnotationsFromJson(row.get("jsonAnnotations"), 
            			 curContribution, row.get("annotationOwnerEmail"));
            	
                /*List<AnnotationDescription> myObjects = Arrays.asList(
                    new ObjectMapper()
                        .readValue(row.get("jsonAnnotations"), AnnotationDescription[].class));
                for (AnnotationDescription annot : myObjects) {
                    AnnotationInstance newAnno = null;
                    if (row.containsKey("annotationOwnerEmail")) {
                        newAnno = this.csvImportApplication.annoService
                            .createTypedAnnotation(annot.annotation);
                        newAnno.setAnnotatorEmail(row.get("annotationOwnerEmail"));
                    } else {
                        newAnno = this.csvImportApplication.annoService
                            .createUnownedTypedAnnotation(annot.annotation);
                    }
                    this.csvImportApplication.annoService.addAnnotation(curContribution, newAnno);
                    this.csvImportApplication.annoService.saveAnnotationInstance(newAnno);
                    ...
                    if (annot.features.size() == 0) {
                        Feature f = this.csvImportApplication.annoService
                            .createFeature(annot.annotation);
                        this.csvImportApplication.annoService.saveFeature(f);
                        this.csvImportApplication.annoService.addFeature(newAnno, f);
                    }
        ...
        if (row.get("replyto") != "") {
            Contribution prior = this.csvImportApplication.getContribution(row.get("replyto"),
                ...
                DataSourceTypes.OTHER, row.get("dataset_name"));
            if (prior != null) {
                this.csvImportApplication.contributionService
                    .createDiscourseRelation(curContribution, prior,
    ...
    ArrayNode parse(String json) {
        try {
            ArrayNode node = new ObjectMapper()
                .readValue(new JsonFactory().createParser(json), ArrayNode.class);
            return node;
        } catch (IOException e) {
