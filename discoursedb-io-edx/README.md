...
While the `save()` method is one of the CRUD methods that all DiscourseDB methods have, the `findBySourceIdAndUsername()` method is specific to the UserRepository and defines a query based on the two fields source_id and username. In case data conversions require these methods, they have to be added to the corresponding Repository in the discoursedb-model project. Documentaion for defining these queries can be found [here](https://docs.spring.io/spring-data/jpa/docs/current/reference/html/#repositories.query-methods.details) and will soon also be available in the documentation of the discoursedb-model project.